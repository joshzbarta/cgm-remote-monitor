...
function init (ctx) {
  ...
  timeago.fullTimeFormat = function fullTimeFormat(opts) {
	if(opts && opts.timeSince) {			
		var d = new Date(opts.timeSince);		
		return {
			value: (d.getMinutes()<10?'0':'')+d.getMinutes()+':'+(d.getSeconds()<10?'0':'')+d.getSeconds(),
			label: 'ago',
			shortLabel: 'ago'				
